                ...
                if (node._def.defaults) {
                    for (d in node._def.defaults) {
                        if (node._def.defaults.hasOwnProperty(d)) {
                            ...
                            if (newValue != null) {
                                //{{{ Added by Homegear GmbH to support multiple inputs - copied from outputs block below. This block parses the inputs property. Important if the property is not a number (e. g. in modbus-out)
                                //    The word "output" was replaced with the word "input".
                                if (d === "inputs") {
                                    if  (newValue.trim() === "") {
                                        continue;
                                    }
                                    if (isNaN(newValue)) {
                                        editState.inputMap = JSON.parse(newValue);
                                        var inputCount = 0;
                                        var inputsChanged = false;
                                        var keys = Object.keys(editState.inputMap);
                                        keys.forEach(function(p) {
                                            if (isNaN(p)) {
                                                // New input;
                                                inputCount ++;
                                                delete editState.inputMap[p];
                                            } else {
                                                editState.inputMap[p] = editState.inputMap[p]+"";
                                                if (editState.inputMap[p] !== "-1") {
                                                    inputCount++;
                                                    if (editState.inputMap[p] !== p) {
                                                        // Input moved
                                                        inputsChanged = true;
                                                    } else {
                                                        delete editState.inputMap[p];
