 ...
 *
 * Todo after update:
 * - Search for "inputs", "outputs" and "target" and make the necessary changes. Add necessary code for input ports.
...
RED.view = (function() {
    ...
    function init() {
            ...
            drop: function( event, ui ) {
                ...
                try {
                    ...
                    if (hideLabel) {
                        ...
                        nn.h = Math.max(node_height,(nn.outputs || 0) * 20); //Added by Homegear GmbH
                    } else {
                        nn.w = Math.max(node_width,20*(Math.ceil((labelParts.width+50+(nn._def.inputs>0?7:0))/20)) );
                        //nn.h = Math.max(6+24*labelParts.lines.length,(nn.outputs || 0) * 15, 30); //Removed by Homegear GmbH
                        nn.h = Math.max(6+24*labelParts.lines.length,(nn.outputs || 0) * 20, 30); //Added by Homegear GmbH
                    }
                } catch(err) {
                }
        ...
        RED.view.annotations.register("red-ui-flow-node-fixed-input",{
            type: "badge",
            class: "red-ui-flow-node-fixed-input",
            element: function(d) {
                var fixedInputBadge = document.createElementNS("http://www.w3.org/2000/svg","path");
                fixedInputBadge.setAttribute("d","M 0,9 l 10,0 -5,-8 z");
                return fixedInputBadge
            },
            show: function(n) { return n.fixedInputs }
        })
        //}}}
    ...
    function showQuickAddDialog(options) {
        ...
        var rebuildQuickAddLink = function() {
            ...
            var numOutputs = (quickAddLink.portType === PORT_TYPE_OUTPUT)?(quickAddLink.node.outputs || 1):(quickAddLink.node.inputs || 1); //Modified by Homegear GmbH - added inputs
                    ...
                    if (selectedLinks.length() > 0) {
                        selectedLinks.forEach(function(link) {
                            ...
                            (mousedown_port_type === PORT_TYPE_INPUT &&
                                ...
                                link.targetPort === mousedown_port_index //Added by Homegear GmbH to support multiple inputs
                            ))) {
    ...
    function updateSelection() {
        ...
        var selectionJSON = activeWorkspace+":"+JSON.stringify(selection,function(key,value) {
            if (key === 'nodes' || key === 'flows') {
                ...
                return value.source.id+":"+value.sourcePort+":"+value.target.id+":"+value.targetPort; //Added by Homegear GmbH to support multiple inputs
            } else if (key === 'links') {
                return value.map(function(link) {
                    //return link.source.id+":"+link.sourcePort+":"+link.target.id; //Removed by Homegear GmbH
                    return link.source.id+":"+link.sourcePort+":"+link.target.id+":"+link.targetPort; //Added by Homegear GmbH to support multiple inputs
                });
            }
            return value;
        });
        ...
        if (showAllLinkPorts !== -1 || mouse_mode != RED.state.JOINING) {
            ...
            node.each(function(d,i) {
                ...
                if (d.dirty) {
                    ...
                    var labelParts;
                    if (d.resize || this.__hideLabel__ !== hideLabel || this.__label__ !== label || this.__inputs__.length !== d.inputs || this.__outputs__.length !== d.outputs) { //Modified by Homegear GmbH - added inputs
                    ...
                    if (mouse_mode != RED.state.MOVING_ACTIVE) {
                        ...
                        var yp = d.h / 2 - (this.__labelLineCount__ / 2) * 24 + 13; //Modified by Homegear GmbH: Keep 13 here for middle vertical alignment of label in node
                        ...
                        for(let portIndex = 0; portIndex < numInputs; portIndex++ ) {
                            ...
                            portGroup.setAttribute("transform","translate("+x+","+((y+18*portIndex)-5)+")")
            ...
            group.each(function(d,i) {
                ...
                if (d.dirty || dirtyGroups[d.id]) {
                    ...
                    if (d.style.label) {
                        ...
                        if (labelPos[0] === 'n') {
                            //labelY = 0+15; // Allow for font-height //Removed by Homegear GmbH
                            labelY = 0+20; // Allow for font-height //Added by Homegear GmbH
                        } else {
    ...
    function calculateNodeDimensions(node) {
        ...
        var labelParts = getLabelParts(label, "red-ui-flow-node-label");
        if (hideLabel) {
            //result[1] = Math.max(node_height,(node.outputs || 0) * 15); //Removed by Homegear GmbH
            result[1] = Math.max(node_height,(node.outputs || 0) * 20); //Added by Homegear GmbH
        } else {
            //result[1] = Math.max(6+24*labelParts.lines.length,(node.outputs || 0) * 15, 30); //Removed by Homegear GmbH
            result[1] = Math.max(6+24*labelParts.lines.length,(node.outputs || 0) * 20, 30); //Added by Homegear GmbH
        }
        if (hideLabel) {
